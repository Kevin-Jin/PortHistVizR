...
load.schwab.transactions <- function(export.file) {
  security.actions <- data.frame(
    ...
    Sign=c(
      ...
      +1, +1, +1, +1, +1))
  cash.actions <- c(
    "MoneyLink Transfer", "MoneyLink Adj", "Bank Interest", "Bank Interest Adj", "Cash Dividend",
    "Pr Yr Cash Div", "Service Fee", "Margin Interest", "Long Term Cap Gain", "Short Term Cap Gain",
    "Wire Sent", "Wire Received", "Misc Cash Entry")
  ...
  tx$Reference.Symbol <- ifelse(
    tx$Action %in% c(
      "Cash Dividend", "Pr Yr Cash Div", "Long Term Cap Gain", "Short Term Cap Gain"),
    tx$Symbol,
    "")
  tx$Reference.Symbol[
    tx$Action %in% c("Bank Interest", "Bank Interest Adj", "Margin Interest")] <- "$"
